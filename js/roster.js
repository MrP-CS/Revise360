...
(function () {
  ...
  function printCards(students, schoolCode) {
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Revise 360 login cards</title>
      <style>
      ...
      <p class="note">Cut along the dashed lines and hand one to each student. PINs cannot be looked up later: reprint from your dashboard or reset a PIN there.</p>
  ...
  function render(host, data) {
    ...
    host.innerHTML = `
      ...
      <div id="newcards">${lastCreated ? `<div class="callout"><p><b>${lastCreated.students.length} login${lastCreated.students.length === 1 ? "" : "s"} ready.</b> Print the cards now, or print a whole class from the list below at any time.</p>
        <p><button class="btn" id="pnow">🖨 Print these cards</button> <button class="btn ghost" id="pdone">Done</button></p></div>` : ""}</div>
      ...
      ${Object.keys(byClass).sort().map(cls => `
        <h3 style="margin:18px 0 6px">${esc(cls)} <span class="muted" style="font-weight:400">· ${byClass[cls].length} student${byClass[cls].length === 1 ? "" : "s"}</span>
          <button class="btn small ghost" data-print="${esc(cls)}">🖨 Print cards</button></h3>
