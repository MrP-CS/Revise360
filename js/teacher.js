(async function () {
  ...
  const exps = {};
  for (const e of reg.experiences) { try { exps[e.id] = await (await fetch("experiences/" + e.id + ".json")).json(); } catch (x) {} }
  const COL = { g: "#50dc96", a: "#ffd046", r: "#ff5f5f", n: "#51607a" };
  let rows = [], students = [], cls = "", expSel = reg.experiences[0]?.id;
  ...
  function render() {
    ...
    $("#main").innerHTML = `
      <div class="row" style="justify-content:space-between">
        <div class="row"><label for="cf" class="muted">Class</label><select id="cf" class="btn small ghost">${['<option value="">All classes</option>', ...classes.map(c => `<option ${c === cls ? "selected" : ""}>${esc(c)}</option>`)].join("")}</select></div>
  ...
  function csv(list) {
    const out = [["Student", "Class", "Lesson", "Experience", "Scene", "Station", "Marks", "Out of", "Band", "Mistakes", "Fixed in review"]];
    list.forEach(s => reg.experiences.forEach(e => (s.exps[e.id]?.stations || []).forEach(st => {
      if (st.done) out.push([s.name, s.cls, e.lesson, e.title, st.sceneTitle, st.name, st.got, st.tot, Store.BAND_LABEL[st.band], st.wrongTasks, st.fixed]);
    })));
    ...
    a.download = "network-vr-results.csv"; document.body.appendChild(a); a.click(); a.remove();
  }
  load();
})();
